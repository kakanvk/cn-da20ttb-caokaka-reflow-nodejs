...
function UpdateSinger() {
    ...
    const handleUpdateById = (newData) => {
        axios.put(`http://localhost:3005/api/singers/${id}`, newData, {
    ...
    const fecthUserData = async () => {
        setSpinning(true);
        axios.get(`http://localhost:3005/api/singers/${id}`, {
            withCredentials: true
        })
            .then(response => {
                console.log(response.data);
    ...
    return (
        <Flex className="UpdateSinger" vertical gap={20}>
            {contextHolder}
            ...
            <Breadcrumb
                items={[
                    ...
                    {
                        title: <Link to="/admin/singers">Quản lý ca sĩ</Link>,
                    },
                    {
                        title: 'Cập nhật',
                    },
